...
module Google
  module Cloud
    module Chronicle
      ...
      ##
      # Create a new client object for FeedsService.
      #
      # By default, this returns an instance of
      # [Google::Cloud::Chronicle::V1::FeedsService::Client](https://cloud.google.com/ruby/docs/reference/google-cloud-chronicle-v1/latest/Google-Cloud-Chronicle-V1-FeedsService-Client)
      # for a gRPC client for version V1 of the API.
      # However, you can specify a different API version by passing it in the
      # `version` parameter. If the FeedsService service is
      # supported by that API version, and the corresponding gem is available, the
      # appropriate versioned client will be returned.
      # You can also specify a different transport by passing `:rest` or `:grpc` in
      # the `transport` parameter.
      #
      # Raises an exception if the currently installed versioned client gem for the
      # given API version does not support the given transport of the FeedsService service.
      # You can determine whether the method will succeed by calling
      # {Google::Cloud::Chronicle.feeds_service_available?}.
      #
      # ## About FeedsService
      #
      ...
      # @return [::Object] A client object for the specified version.
      #
      def self.feeds_service version: :v1, transport: :grpc, &block
        require "google/cloud/chronicle/#{version.to_s.downcase}"
        ...
        package_name = Google::Cloud::Chronicle
                       ...
                       .select { |sym| sym.to_s.downcase == version.to_s.downcase.tr("_", "") }
                       .first
        service_module = Google::Cloud::Chronicle.const_get(package_name).const_get(:FeedsService)
        service_module = service_module.const_get(:Rest) if transport == :rest
        service_module.const_get(:Client).new(&block)
      end

      ##
      # Determines whether the FeedsService service is supported by the current client.
      # If true, you can retrieve a client object by calling {Google::Cloud::Chronicle.feeds_service}.
      # If false, that method will raise an exception. This could happen if the given
      # API version does not exist or does not support the FeedsService service,
      # or if the versioned client gem needs an update to support the FeedsService service.
      #
      # @param version [::String, ::Symbol] The API version to connect to. Optional.
      #   Defaults to `:v1`.
      # @param transport [:grpc, :rest] The transport to use. Defaults to `:grpc`.
      # @return [boolean] Whether the service is available.
      #
      def self.feeds_service_available? version: :v1, transport: :grpc
        require "google/cloud/chronicle/#{version.to_s.downcase}"
        package_name = Google::Cloud::Chronicle
                       .constants
                       .select { |sym| sym.to_s.downcase == version.to_s.downcase.tr("_", "") }
        ...
        return false unless package_name
        service_module = Google::Cloud::Chronicle.const_get package_name
        return false unless service_module.const_defined? :FeedsService
        service_module = service_module.const_get :FeedsService
        if transport == :rest
          return false unless service_module.const_defined? :Rest
          service_module = service_module.const_get :Rest
        end
