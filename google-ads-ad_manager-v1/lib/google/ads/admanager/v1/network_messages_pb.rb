# frozen_string_literal: true
# Generated by the protocol buffer compiler.  DO NOT EDIT!
# source: google/ads/admanager/v1/network_messages.proto

require 'google/protobuf'

require 'google/ads/admanager/v1/third_party_data_declaration_pb'
require 'google/api/field_behavior_pb'
require 'google/api/resource_pb'


descriptor_data = "\n.google/ads/admanager/v1/network_messages.proto\x12\x17google.ads.admanager.v1\x1a:google/ads/admanager/v1/third_party_data_declaration.proto\x1a\x1fgoogle/api/field_behavior.proto\x1a\x19google/api/resource.proto\"\xd0\x04\n\x07Network\x12\x11\n\x04name\x18\x01 \x01(\tB\x03\xe0\x41\x08\x12\x1e\n\x0c\x64isplay_name\x18\x02 \x01(\tB\x03\xe0\x41\x01H\x00\x88\x01\x01\x12\x1e\n\x0cnetwork_code\x18\x03 \x01(\tB\x03\xe0\x41\x03H\x01\x88\x01\x01\x12\x1f\n\rproperty_code\x18\x04 \x01(\tB\x03\xe0\x41\x03H\x02\x88\x01\x01\x12\x1b\n\ttime_zone\x18\x05 \x01(\tB\x03\xe0\x41\x03H\x03\x88\x01\x01\x12\x1f\n\rcurrency_code\x18\x06 \x01(\tB\x03\xe0\x41\x03H\x04\x88\x01\x01\x12%\n\x18secondary_currency_codes\x18\x07 \x03(\tB\x03\xe0\x41\x01\x12L\n\x16\x65\x66\x66\x65\x63tive_root_ad_unit\x18\x08 \x01(\tB\'\xe0\x41\x03\xfa\x41!\n\x1f\x61\x64manager.googleapis.com/AdUnitH\x05\x88\x01\x01\x12\x1e\n\x0ctest_network\x18\n \x01(\x08\x42\x03\xe0\x41\x03H\x06\x88\x01\x01\x12\x1c\n\nnetwork_id\x18\x0b \x01(\x03\x42\x03\xe0\x41\x03H\x07\x88\x01\x01:Q\xea\x41N\n admanager.googleapis.com/Network\x12\x17networks/{network_code}*\x08networks2\x07networkB\x0f\n\r_display_nameB\x0f\n\r_network_codeB\x10\n\x0e_property_codeB\x0c\n\n_time_zoneB\x10\n\x0e_currency_codeB\x19\n\x17_effective_root_ad_unitB\x0f\n\r_test_networkB\r\n\x0b_network_id\"\xfb\x02\n DefaultThirdPartyDataDeclaration\x12\x11\n\x04name\x18\x01 \x01(\tB\x03\xe0\x41\x08\x12\x62\n\x1cthird_party_data_declaration\x18\x02 \x01(\x0b\x32\x32.google.ads.admanager.v1.ThirdPartyDataDeclarationB\x03\xe0\x41\x01H\x00\x88\x01\x01:\xbe\x01\xea\x41\xba\x01\n9admanager.googleapis.com/DefaultThirdPartyDataDeclaration\x12\x38networks/{network_code}/defaultThirdPartyDataDeclaration*!defaultThirdPartyDataDeclarations2 defaultThirdPartyDataDeclarationB\x1f\n\x1d_third_party_data_declarationB\xc8\x01\n\x1b\x63om.google.ads.admanager.v1B\x14NetworkMessagesProtoP\x01Z@google.golang.org/genproto/googleapis/ads/admanager/v1;admanager\xaa\x02\x17Google.Ads.AdManager.V1\xca\x02\x17Google\\Ads\\AdManager\\V1\xea\x02\x1aGoogle::Ads::AdManager::V1b\x06proto3"

pool = ::Google::Protobuf::DescriptorPool.generated_pool
pool.add_serialized_file(descriptor_data)

module Google
  module Ads
    module AdManager
      module V1
        Network = ::Google::Protobuf::DescriptorPool.generated_pool.lookup("google.ads.admanager.v1.Network").msgclass
        DefaultThirdPartyDataDeclaration = ::Google::Protobuf::DescriptorPool.generated_pool.lookup("google.ads.admanager.v1.DefaultThirdPartyDataDeclaration").msgclass
      end
    end
  end
end
