...
describe "#get_param_version", :parameter_manager_snippet do
  before do
  ...
  it "get a parameter version" do
    sample = SampleLoader.load "get_param_version.rb"

    out, _err = capture_io do
      sample.run project_id: project_id, parameter_id: parameter_id, version_id: version_id
    end

    assert_equal "Found parameter version projects/#{project_id}/locations/global/parameters/" \
                 "#{parameter_id}/versions/#{version_id} with state enabled\nPayload: #{payload}\n",
                 out
  end
end
