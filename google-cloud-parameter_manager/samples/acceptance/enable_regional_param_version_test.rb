...
describe "#enable_regional_param_version", :regional_parameter_manager_snippet do
  before do
  ...
  it "enable a regional parameter versions" do
    sample = SampleLoader.load "enable_regional_param_version.rb"

    out, _err = capture_io do
      sample.run project_id: project_id, location_id: location_id, parameter_id: parameter_id, version_id: version_id
    end

    assert_equal "Enabled regional parameter version #{version_id} for parameter #{parameter_id}\n", out
  end
end
