...
describe Gcloud::Logging::Entry, :to_grpc, :mock_logging do
  ...
  it "returns the correct data when empty" do
    ...
    Gcloud::GRPCUtils.map_to_hash(grpc.labels).must_be :empty?
  ...
  it "returns the correct data when data is added" do
    ...
    Gcloud::GRPCUtils.map_to_hash(grpc.resource.labels).must_equal({ "description" => "The server is running in test",
                                                                     "env"         => "test",
                                                                     "valueType"   => "STRING" })
    ...
    Gcloud::GRPCUtils.map_to_hash(grpc.labels).must_equal("env" => "test", "fizz" => "buzz")
