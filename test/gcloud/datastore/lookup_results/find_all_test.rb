...
describe Gcloud::Datastore::Dataset, :find_all do
  ...
  let(:key2) { Gcloud::Datastore::Key.new "ds-test", "thingie2" }
  let(:key3) { Gcloud::Datastore::Key.new "ds-test", "thingie3" }
  let(:key4) { Gcloud::Datastore::Key.new "ds-test", "thingie4" }
  let(:key5) { Gcloud::Datastore::Key.new "ds-test", "thingie5" }
  let(:key6) { Gcloud::Datastore::Key.new "ds-test", "thingie6" }
  ...
  let(:first_lookup_res) do
    Google::Datastore::V1beta3::LookupResponse.new(
      found: (1..2).map do |i|
        Google::Datastore::V1beta3::EntityResult.new(
          entity: Google::Datastore::V1beta3::Entity.new(
            key: Gcloud::Datastore::Key.new("ds-test", "thingie#{i}").to_grpc,
      ...
      deferred: (3..4).map do |i|
        Gcloud::Datastore::Key.new("ds-test", "thingie#{i}").to_grpc
      end
    )
  end
