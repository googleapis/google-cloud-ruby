...
describe Google::Cloud::Bigquery::Dataset, :access, :mock_bigquery do
  ...
  describe :dataset do
    ...
    it "adds an access entry with specifying a dataset object" do
      ...
      bigquery.service.mocked_service = mock
      updated_gapi = dataset_gapi.dup
      new_access = Google::Apis::BigqueryV2::Dataset::Access.new dataset: dataset_1.access_entry(target_types: ["VIEWS"])
      updated_gapi.access = new_access
      patch_gapi = Google::Apis::BigqueryV2::Dataset.new access: [new_access], etag: dataset_gapi.etag
      mock.expect :patch_dataset, updated_gapi, [project, dataset_id, patch_gapi], options: {header: {"If-Match" => dataset_gapi.etag}}

      dataset.access do |acl|
        refute acl.reader_dataset? dataset_1.access_entry(target_types: target_types)
        acl.add_reader_dataset dataset_1.access_entry(target_types: target_types)
        assert acl.reader_dataset? dataset_1.access_entry(target_types: target_types)
        acl.remove_reader_dataset dataset_1.access_entry(target_types: target_types)
        refute acl.reader_dataset? dataset_1.access_entry(target_types: target_types)
        acl.add_reader_dataset dataset_1.access_entry(target_types: target_types) # this entry goes into the request
      end
      mock.verify
    end
    ...
    it "adds an access entry with specifying a dataset hash" do
      ...
      bigquery.service.mocked_service = mock
      updated_gapi = dataset_gapi.dup
      new_access = Google::Apis::BigqueryV2::Dataset::Access.new dataset: dataset_1.access_entry(target_types: target_types)
      updated_gapi.access = new_access
      patch_gapi = Google::Apis::BigqueryV2::Dataset.new access: [new_access], etag: dataset_gapi.etag
      mock.expect :patch_dataset, updated_gapi, [project, dataset_id, patch_gapi], options: {header: {"If-Match" => dataset_gapi.etag}}
