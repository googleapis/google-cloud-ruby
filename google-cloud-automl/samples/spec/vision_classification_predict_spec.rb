...
xdescribe "Vision Classification Predict" do
  let(:project_id) { ENV["AUTOML_PROJECT_ID"] }
  let(:model_id) { ENV["AUTOML_VISION_CLASS_MODEL_ID"] }
  ...
  example "Predict" do
    ...
    capture do
      classification_predict actual_project_id: project_id, actual_model_id: model_id, actual_file_path: file_path
    end
  ...
  example "Batch Predict", :slow do
    salad_url = ensure_import_file! bucket, "test.png"
    input_io = StringIO.new(salad_url)
    input_file = ensure_import_file! bucket, "batch-vision_class.csv", input_io
