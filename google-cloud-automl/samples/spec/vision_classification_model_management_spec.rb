...
xdescribe "Vision Classification Model Management" do
  let(:project_id) { ENV["AUTOML_PROJECT_ID"] }
  let(:dataset_id) { ENV["AUTOML_VISION_CLASS_DATASET_ID"] }
  ...
  example "Create a model", :slow do
    ...
    capture do
      classification_create_model actual_project_id: project_id, actual_dataset_id: dataset_id,
  ...
  example "Undeploy and deploy a model with node count", :slow do
    ...
    capture do
      classification_deploy_model_node_count actual_project_id: project_id, actual_model_id: model_id
    end
    expect(captured_output).to include "Model deployment finished."
  end
end
