...
class MethodMapping
  ...
  def self.get
  {
    "storage.bucket_acl.list" => [:list_bucket_acls],  # S1 start
  ...
  def blob_download_as_bytes_w_range client, preconditions, **resources
    ...
    assert_equal stored_contents, data.encode("utf-8")[start_byteend_byte]
  end

  def bucket_delete_blob client, preconditions, **resources
  ...
  def bucket_delete_blobs client, preconditions, **resources
    object = resources.get("object")
    bucket = client.bucket(resources.get("bucket").name)
    sources = [object]
  ...
  def blob_delete client, preconditions, **resources
    bucket = resources.get("bucket")
    object = resources.get("object")
  ...
  def blob_update client, preconditions, **resources
    bucket = resources.get("bucket")
    object = resources.get("object")
    blob = client.bucket(bucket.name).blob(object.name)
  ...
  def bucket_copy_blob client, preconditions, **resources
    object = resources.get("object")
    bucket = client.bucket(resources.get("bucket").name)
  ...
  def bucket_rename_blob client, preconditions, **resources
    object = resources.get("object")
    bucket = client.bucket(resources.get("bucket").name)
  ...
  def blob_rewrite client, preconditions, **resources
    bucket = resources.get("bucket")
    object = resources.get("object")
  ...
  def blob_update_storage_class client, preconditions, **resources
    bucket = resources.get("bucket")
    object = resources.get("object")
    blob = client.bucket(bucket.name).blob(object.name)
  ...
  def blob_compose client, preconditions, **resources
    bucket = resources.get("bucket")
    object = resources.get("object")
    blob = client.bucket(bucket.name).blob(object.name)
  ...
  def blob_upload_from_string client, preconditions, **resources
    bucket = resources.get("bucket")
  ...
  def blob_upload_from_file client, preconditions, **resources
    bucket = resources.get("bucket")
    file, data = resources.get("file_data")
    ...
    def blob_upload_from_filename client, preconditions, **resources
      bucket = resources.get("bucket")
      blob = client.bucket(bucket.name).blob(uuid.uuid4().hex)
      blob.chunk_size = _RESUMABLE_UPLOAD_CHUNK_SIZE
  ...
  def blobwriter_write client, preconditions, **resources
    bucket = resources.get("bucket")
    _, data = resources.get("file_data")
    blob = client.bucket(bucket.name).blob(uuid.uuid4().hex)
    if preconditions
      ...
      with blob.open("w", chunk_size=_RESUMABLE_UPLOAD_CHUNK_SIZE) 
      writer.write(data)
      blob.reload()
      assert blob.size == len(data)
    end
  ...
  def blobwriter_write_multipart client, preconditions, **resources
    chunk_size = 256 * 1024
    bucket = resources.get("bucket")
    blob = client.bucket(bucket.name).blob(uuid.uuid4().hex)
    if preconditions
      with blob.open("wb", chunk_size=chunk_size, if_generation_match=0) 
      writer.write(_BYTE_CONTENT)
    else
      with blob.open("wb", chunk_size=chunk_size) 
      writer.write(_BYTE_CONTENT)
    end
  end


  def blob_upload_from_string_multipart client, preconditions, **resources
    bucket = resources.get("bucket")
    blob = client.bucket(bucket.name).blob(uuid.uuid4().hex)
    if preconditions
  ...
  def blob_create_resumable_upload_session client, preconditions, **resources
    bucket = resources.get("bucket")
    blob = client.bucket(bucket.name).blob(uuid.uuid4().hex)
    if preconditions
