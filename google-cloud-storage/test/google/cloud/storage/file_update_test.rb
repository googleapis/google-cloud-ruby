...
describe Google::Cloud::Storage::File, :update, :mock_storage do
  ...
  it "updates its cache control" do
    ...
    mock.expect :patch_object, file_gapi, [bucket_name, file.name, patch_file_gapi], **patch_object_args
  ...
  it "updates its storage_class" do
    ...
    mock.expect :rewrite_object, done_rewrite(patched_file_gapi),
      [bucket_name, file.name, bucket_name, file.name, patch_file_gapi], **rewrite_object_args
  ...
  it "updates its storage_class with user_project set to true" do
    ...
    mock.expect :rewrite_object, done_rewrite(patched_file_gapi),
      [bucket_name, file.name, bucket_name, file_user_project.name, patch_file_gapi], **rewrite_object_args(user_project: "test")
  ...
  it "updates its storage_class, calling rewrite_object as many times as is needed" do
    ...
    mock.expect :rewrite_object, undone_rewrite("keeptrying"),
      [bucket_name, file.name, bucket_name, file.name, patch_file_gapi], **rewrite_object_args(rewrite_token: "notyetcomplete")
    mock.expect :rewrite_object, undone_rewrite("almostthere"),
      [bucket_name, file.name, bucket_name, file.name, patch_file_gapi], **rewrite_object_args(rewrite_token: "keeptrying")
    mock.expect :rewrite_object, done_rewrite(patched_file_gapi),
      [bucket_name, file.name, bucket_name, file.name, patch_file_gapi], **rewrite_object_args(rewrite_token: "almostthere")
  ...
  it "updates multiple attributes in a block with user_project set to true" do
    ...
    mock.expect :patch_object, file_gapi, [bucket_name, file.name, patch_file_gapi], **patch_object_args(user_project: "test")
  ...
  it "updates with generation" do
    mock = Minitest::Mock.new
    patch_file_gapi = Google::Apis::StorageV1::Object.new content_language: "de"
    mock.expect :patch_object, file_gapi, [bucket_name, file.name, patch_file_gapi], **patch_object_args(generation: generation)
    file.service.mocked_service = mock
  ...
  it "updates with if_generation_match" do
    mock = Minitest::Mock.new
    patch_file_gapi = Google::Apis::StorageV1::Object.new content_language: "de"
    mock.expect :patch_object, file_gapi, [bucket_name, file.name, patch_file_gapi], **patch_object_args(if_generation_match: generation)
    file.service.mocked_service = mock
  ...
  it "updates with if_generation_not_match" do
    mock = Minitest::Mock.new
    patch_file_gapi = Google::Apis::StorageV1::Object.new content_language: "de"
    mock.expect :patch_object, file_gapi, [bucket_name, file.name, patch_file_gapi], **patch_object_args(if_generation_not_match: generation)
    file.service.mocked_service = mock
  ...
  it "updates with if_metageneration_not_match" do
    mock = Minitest::Mock.new
    patch_file_gapi = Google::Apis::StorageV1::Object.new content_language: "de"
    mock.expect :patch_object, file_gapi, [bucket_name, file.name, patch_file_gapi], **patch_object_args(if_metageneration_not_match: metageneration)
    file.service.mocked_service = mock
