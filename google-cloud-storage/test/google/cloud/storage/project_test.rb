...
describe Google::Cloud::Storage::Project, :mock_storage do
  ...
  describe "storage encryption enforcement config" do
    # set and get customer managed encryption enforcement config and verify
    it "sets and gets customer managed bucket encryption enforcement config" do
      mock = Minitest::Mock.new
      created_bucket = create_bucket_gapi bucket_name
      created_bucket.encryption = encryption_gapi(customer_managed_config_restriction_mode: "NotRestricted")
      resp_bucket = bucket_with_location created_bucket
      mock.expect :insert_bucket, resp_bucket, [project, created_bucket], predefined_acl: nil, predefined_default_object_acl: nil, user_project: nil, enable_object_retention: nil, options: {}
      storage.service.mocked_service = mock
      bucket = storage.create_bucket bucket_name do |b|
        # set customer managed encryption enforcement config 
        b.customer_managed_encryption_enforcement_config = customer_managed_config(restriction_mode: "NotRestricted")
      end
      mock.verify
      # get customer managed encryption enforcement config and verify
    ...
    # set customer supplied and google managed encryption enforcement config and verify
    it "sets customer supplied and google managed bucket encryption enforcement config" do
      mock = Minitest::Mock.new
      created_bucket = create_bucket_gapi bucket_name
      created_bucket.encryption = encryption_gapi(customer_supplied_config_restriction_mode: "FullyRestricted", google_managed_config_restriction_mode: "NotRestricted" )
      resp_bucket = bucket_with_location created_bucket
      mock.expect :insert_bucket, resp_bucket, [project, created_bucket], predefined_acl: nil, predefined_default_object_acl: nil, user_project: nil, enable_object_retention: nil, options: {}
      storage.service.mocked_service = mock

      # set customer supplied and google managed encryption enforcement config
      bucket = storage.create_bucket bucket_name do |b|
        b.customer_supplied_encryption_enforcement_config = customer_supplied_config(restriction_mode: "FullyRestricted")
        b.google_managed_encryption_enforcement_config = google_managed_config(restriction_mode: "NotRestricted" )
      end
      mock.verify
      # get customer supplied and google managed encryption enforcement config and verify
    ...
    # delete customer supplied encryption enforcement config and verify
    it "deletes customer supplied encryption enforcement config" do
      mock = Minitest::Mock.new
      
      created_bucket = create_bucket_gapi bucket_name
      # Set initial state: FullyRestricted
      created_bucket.encryption = encryption_gapi(customer_supplied_config_restriction_mode: "FullyRestricted")
      resp_bucket = bucket_with_location created_bucket

      mock.expect :insert_bucket, resp_bucket do |project_id, b_arg, **kwargs|
        project_id == project && 
        b_arg.encryption.customer_supplied_encryption_enforcement_config.restriction_mode == "FullyRestricted"
      end

      # Define what the bucket should look like AFTER the delete (config should be nil)
      updated_bucket_gapi = create_bucket_gapi bucket_name
      updated_bucket_gapi.encryption = encryption_gapi(customer_supplied_config_restriction_mode: nil)

      # We verify that the patch_bucket call is made with a bucket that has NO config
      mock.expect :patch_bucket, updated_bucket_gapi do |name, b_arg, **kwargs|
        name == bucket_name && 
        b_arg.encryption.customer_supplied_encryption_enforcement_config.nil?
      end
      storage.service.mocked_service = mock
