...
describe "Create reference image", :product_search do
  it "creates reference image" do
    ...
    reference_images_before = Array(@client.list_reference_images(parent: product.name))
    _(reference_images_before.length).must_equal 0

    `ruby #{snippet_filepath} #{@project_id} #{@location} #{product_id}`

    reference_images_after = Array(@client.list_reference_images(parent: product.name))
