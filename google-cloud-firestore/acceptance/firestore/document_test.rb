...
describe "Document", :firestore do
  ...
  it "has set() method" do
    ...
    doc_snp[:linked].must_be_kind_of Google::Cloud::Firestore::Document::Reference
  ...
  it "supports server timestamps for merge" do
    ...
    doc_ref.set({ a: :b })
    doc_ref.set({ c: :SERVER_TIME }, merge: true)
  ...
  it "has update method" do
    ...
    commit_time = doc_ref.set({ foo: "a" })
    doc_ref.update({ foo: "b" }, update_time: commit_time)
  ...
  it "can add and delete fields sequentially" do
    ...
    # doc_ref.update({ a: { d: :DELETE } })
    doc_ref.update({ "a.d" => :DELETE })
    doc_ref.get.data.must_equal({ a: { b: "c" } })

    # INFO: the original test used nested deletes, but that is not allowed...
    # doc_ref.update({ a: { b: :DELETE } })
    doc_ref.update({ "a.b" => :DELETE })
    ...
    doc_ref.update({ i: :DELETE })
    doc_ref.get.data.must_equal({ a: { e: "foo" } })

    doc_ref.update({ a: :DELETE })
    doc_ref.get.data.must_equal({})
  end
  ...
  it "can add and delete fields with server timestamps" do
    ...
    doc_ref.create({ time: :SERVER_TIME, a: { b: :SERVER_TIME } })
    doc_snp = doc_ref.get
    ...
    doc_ref.set({ time: :SERVER_TIME, a: { c: :SERVER_TIME } })
    doc_snp = doc_ref.get
    doc_snp[:time].must_be_kind_of Time
    times << doc_snp[:time]
    ...
    doc_ref.set({ time: :SERVER_TIME, a: { d: :SERVER_TIME } }, merge: true)
    doc_snp = doc_ref.get
    doc_snp[:time].must_be_kind_of Time
    times << doc_snp[:time]
    doc_snp[:a][:b].must_be :nil?
    ...
    doc_ref.set({ time: :SERVER_TIME, e: :SERVER_TIME }, merge: true)
    doc_snp = doc_ref.get
    doc_snp[:time].must_be_kind_of Time
    times << doc_snp[:time]
    doc_snp[:a][:b].must_be :nil?
    ...
    doc_ref.set({ time: :SERVER_TIME, e: { f: :SERVER_TIME } }, merge: true)
    doc_snp = doc_ref.get
    doc_snp[:time].must_be_kind_of Time
    times << doc_snp[:time]
    doc_snp[:a][:b].must_be :nil?
    ...
    doc_ref.update({ time: :SERVER_TIME, "g.h" => :SERVER_TIME })
    doc_snp = doc_ref.get
    doc_snp[:time].must_be_kind_of Time
    times << doc_snp[:time]
    doc_snp[:a][:b].must_be :nil?
    ...
    doc_ref.update({ time: :SERVER_TIME, "g.j.k" => :SERVER_TIME })
    doc_snp = doc_ref.get
    doc_snp[:time].must_be_kind_of Time
    times << doc_snp[:time]
    doc_snp[:a][:b].must_be :nil?
