...
describe "Query Explain", :firestore_acceptance do
  ...
  it "runs a query with explain (analyze: false) - returns only metrics, no results" do
    ...
    results = rand_query_col.explain.get
  ...
  it "runs a query with explain (analyze: true) - returns both results and metrics" do
    ...
    results = rand_query_col.explain(analyze: true).get
  ...
  it "works with limit and analyze: true" do
    ...
    results = rand_query_col.order(:population).limit(2).explain(analyze: true).get
  ...
  it "works with limit_to_last and analyze: true" do
    ...
    results = rand_query_col.order(:population).limit_to_last(2).explain(analyze: true).get
  ...
  it "handles empty result sets with analyze: true" do
    ...
    results = rand_query_col.where(:population, :>, 1000).explain(analyze: true).get
  ...
  it "works with query method call order variations" do
    rand_query_col = firestore.col "#{root_path}/query/#{SecureRandom.hex(4)}"
    # Add some documents
    rand_query_col.add({name: "Alice", population: 100})
    rand_query_col.add({name: "Bob", population: 200})
    ...
    # Test different method ordering: explain first
    results1 = rand_query_col.explain(analyze: true).order(:population).limit(2).get
    _(results1.count).must_equal 2
    _(results1.explain_metrics).wont_be_nil
    
    # Test different method ordering: explain last
    results2 = rand_query_col.order(:population).limit(2).explain(analyze: true).get
    _(results2.count).must_equal 2
    _(results2.explain_metrics).wont_be_nil
    
    # Both queries should return the same results
    names1 = results1.map { |doc| doc[:name] }
    names2 = results2.map { |doc| doc[:name] }
    _(names1).must_equal names2
  end
  ...
  it "doesn't produce different results from queries without explain" do
    ...
    results_with_explain = rand_query_col.order(:population).explain(analyze: true).get
