...
describe Google::Cloud::Firestore::Database, :run, :mock_firestore do
  ...
  it "gets a document (doc ref)" do
    ...
    firestore_mock.expect :batch_get_documents, [get_doc_resp].to_enum, ["projects/#{project}/databases/(default)", ["projects/#{project}/databases/(default)/documents/users/mike"], mask: nil, options: default_options]
    ...
    doc.parent.project_id.must_equal col.project_id
  ...
  it "runs a collection (ref)" do
    ...
    col = firestore.col :users
    results_enum = firestore.run col

    assert_results_enum results_enum
  end

  it "runs a collection (string)" do
    expected_query = Google::Firestore::V1beta1::StructuredQuery.new(
      from: [Google::Firestore::V1beta1::StructuredQuery::CollectionSelector.new(collection_id: "users", all_descendants: false)]
    )
    firestore_mock.expect :run_query, query_results_enum, ["projects/#{project}/databases/(default)/documents", structured_query: expected_query, options: default_options]

    results_enum = firestore.run :users

    assert_results_enum results_enum
  end

  it "runs a collection (symbol)" do
    expected_query = Google::Firestore::V1beta1::StructuredQuery.new(
      from: [Google::Firestore::V1beta1::StructuredQuery::CollectionSelector.new(collection_id: "users", all_descendants: false)]
    )
    firestore_mock.expect :run_query, query_results_enum, ["projects/#{project}/databases/(default)/documents", structured_query: expected_query, options: default_options]
  ...
  it "runs a simple query" do
    ...
    results_enum = firestore.run query
