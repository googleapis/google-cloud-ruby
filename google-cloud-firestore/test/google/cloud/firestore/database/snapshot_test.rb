...
describe Google::Cloud::Firestore::Database, :snapshot, :mock_firestore do
  ...
  let(:read_time_transaction_opt) do
    Google::Firestore::V1beta1::TransactionOptions.new(
      read_only: Google::Firestore::V1beta1::TransactionOptions::ReadOnly.new(
  ...
  it "runs a simple query" do
    ...
    firestore.snapshot do |s|
      results_enum = s.select(:name).from(:users).run
  ...
  it "runs a complex query" do
    ...
    firestore.snapshot do |s|
      results_enum = s.select(:name).from(:users).offset(3).limit(42).order(:name).order(:__name__, :desc).start_after(:foo).end_before(:bar).run
      assert_results_enum results_enum
    end
  end
  ...
  describe "with read_time" do
    it "runs a simple query" do
      ...
      firestore_mock.expect :run_query, query_results_enum, ["projects/#{project}/databases/(default)/documents", structured_query: expected_query, new_transaction: read_time_transaction_opt, options: default_options]
      firestore_mock.expect :rollback, nil, ["projects/#{project}/databases/(default)", transaction_id, options: default_options]

      firestore.snapshot read_time: read_time do |s|
        results_enum = s.select(:name).from(:users).run
        assert_results_enum results_enum
      end
    end
    ...
    it "runs multiple queries" do
      ...
      firestore_mock.expect :run_query, query_results_enum, ["projects/#{project}/databases/(default)/documents", structured_query: first_query, new_transaction: read_time_transaction_opt, options: default_options]
      firestore_mock.expect :run_query, query_results_enum, ["projects/#{project}/databases/(default)/documents", structured_query: second_query, transaction: transaction_id, options: default_options]
      firestore_mock.expect :rollback, nil, ["projects/#{project}/databases/(default)", transaction_id, options: default_options]
  ...
  def assert_results_enum enum
    ...
    results.each do |result|
      ...
      result.ref.context.must_be_kind_of Google::Cloud::Firestore::Snapshot
      result.parent.context.must_be_kind_of Google::Cloud::Firestore::Snapshot
    end
