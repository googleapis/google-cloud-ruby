...
describe Google::Cloud::Firestore::Snapshot, :collection, :closed, :mock_firestore do
  let(:snapshot) do
  ...
  it "gets a collection by a nested collection path" do
    col = snapshot.col "users/mike/messages"
  ...
  describe "using collection alias" do
    ...
    it "gets a collection by a nested collection path" do
      col = snapshot.collection "users/mike/messages"
    ...
    it "does not allow a document path" do
      error = expect do
        snapshot.collection "users/mike"
      end.must_raise ArgumentError
      error.message.must_equal "collection_path must refer to a collection."
    end
  end
