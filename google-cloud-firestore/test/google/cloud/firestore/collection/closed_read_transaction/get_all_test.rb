...
describe Google::Cloud::Firestore::Database, :get_all, :closed_read_transaction, :mock_firestore do
  ...
  it "gets a single doc (string)" do
    ...
    docs.first.ref.context.must_equal firestore
    docs.first.parent.context.must_equal firestore
  end
  ...
  it "gets multiple docs using splat (string)" do
    firestore_mock.expect :batch_get_documents, batch_docs_enum, [database_path, full_doc_paths, mask: body_mask, options: default_options]
