...
describe Google::Cloud::Firestore::Database, :get_all, :empty_transaction, :mock_firestore do
  ...
  let(:collection) { Google::Cloud::Firestore::Collection.from_path "projects/#{project}/databases/(default)/documents/#{collection_path}", transaction }
  ...
  it "gets multiple docs using splat (string)" do
    firestore_mock.expect :batch_get_documents, batch_docs_enum, [database_path, full_doc_paths, mask: body_mask, new_transaction: transaction_opt, options: default_options]
