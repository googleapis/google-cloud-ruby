...
describe Google::Cloud::Firestore::Collection, :add, :mock_firestore do
  let(:collection_id) { "messages" }
  let(:collection_path) { "users/mike/#{collection_id}" }
  let(:collection) { Google::Cloud::Firestore::Collection.from_path "#{documents_path}/#{collection_path}", firestore }
