...
describe Google::Cloud::Firestore::Batch, :docs, :mock_firestore do
  ...
  it "gets docs with string" do
    ...
    firestore_mock.expect :run_query, query_docs_enum, ["projects/#{project}/databases/(default)/documents", structured_query: expected_query, options: default_options]

    docs_enum = batch.docs "users"
  ...
  it "gets docs with string using documents alias" do
    ...
    docs_enum = batch.documents "users"
  ...
  def assert_docs_enum enum
    ...
    docs.each do |doc|
      ...
      doc.ref.context.must_equal batch
      doc.parent.context.must_equal batch
    end
