...
describe Google::Cloud::Firestore::Batch, :get_all, :mock_firestore do
  ...
  it "gets multiple docs using splat (string)" do
    ...
    docs_enum = batch.get_all "users/mike", "users/tad", "users/chris"
  ...
  it "gets multiple docs using splat (doc ref)" do
    ...
    docs_enum = batch.get_all batch.doc("users/mike"), batch.doc("users/tad"), batch.doc("users/chris")
  ...
  it "gets a single doc (string)" do
    ...
    docs_enum = batch.get_all "users/mike"
  ...
  it "gets a single doc (doc ref)" do
    ...
    docs_enum = batch.get_all batch.doc("users/mike")
    ...
    docs.first.ref.context.must_equal batch
    docs.first.parent.context.must_equal batch
  end

  it "gets multiple docs using splat (string)" do
    firestore_mock.expect :batch_get_documents, batch_docs_enum, [database_path, full_doc_paths, mask: name_mask, options: default_options]

    docs_enum = batch.get_all "users/mike", "users/tad", "users/chris", mask: :name
  ...
  it "gets multiple docs using splat (doc ref)" do
    ...
    docs_enum = batch.get_all batch.doc("users/mike"), batch.doc("users/tad"), batch.doc("users/chris"), mask: :name
