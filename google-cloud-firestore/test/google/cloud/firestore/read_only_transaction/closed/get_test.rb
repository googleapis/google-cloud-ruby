...
describe Google::Cloud::Firestore::ReadOnlyTransaction, :get, :closed, :mock_firestore do
  ...
  it "raises when getting a document (string)" do
    error = expect do
      read_transaction.get "users/mike"
    end.must_raise RuntimeError
    error.message.must_equal "transaction is closed"
  end
