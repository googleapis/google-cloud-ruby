...
describe Google::Cloud::Firestore::ReadOnlyTransaction, :query, :mock_firestore do
  ...
  it "runs a simple query" do
    ...
    results_enum = read_transaction.select(:name).from(:users).run
  ...
  it "runs a simple query as an argument" do
    ...
    results_enum = read_transaction.run query
  ...
  it "runs a complex query" do
    ...
    results_enum = read_transaction.select(:name).from(:users).offset(3).limit(42).order(:name).order(:__name__, :desc).start_after(:foo).end_before(:bar).run
