...
describe Google::Cloud::Firestore::ReadOnlyTransaction, :doc, :mock_firestore do
  ...
  it "produces a Document reference for a document_path" do
    ...
    document = read_transaction.doc document_path
  ...
  it "does not allow a collection_path" do
    error = expect do
      read_transaction.doc "users/mike/messages"
    end.must_raise ArgumentError
    error.message.must_equal "document_path must refer to a document."
  end
end
