...
describe "Cross-Language Set Tests", :mock_firestore do
  ...
  it "basic" do
    ...
    firestore.set document_path, set_data
  end
  ...
  describe :merge do
    it "DELETE cannot appear in an unmerged field" do
      merge_data = { a: 1, b: :DELETE }

      error = expect do
        firestore.set document_path, merge_data, merge: [:a]
      end.must_raise ArgumentError
      error.message.must_equal "DELETE not allowed on set"
    end
    ...
    it "Merge with FieldPaths (array)" do
      ...
      firestore.set document_path, set_data, merge: [["*", "~"]]
    end

    it "Merge with FieldPaths (string)" do
    ...
    it "Merge with a nested field (array)" do
      ...
      firestore.set document_path, set_data, merge: [["h", "g"]]
    end
    ...
    it "Merge field is not a leaf" do
      ...
      firestore.set document_path, set_data, merge: ["h"]
    end

    it "If no ordinary values in Merge, no write" do
      set_data = { a: 1, b: :SERVER_TIME }
      ...
      firestore.set document_path, set_data, merge: "b"
    end
    ...
    it "Merge fields must all be present in data" do
      ...
      error = expect do
        firestore.set document_path, set_data, merge: ["b", "a"]
      end.must_raise ArgumentError
    ...
    it "Merge with a field" do
      ...
      firestore.set document_path, set_data, merge: "a"
    end
    ...
    it "MergeAll cannot be specified with empty data" do
      ...
      error = expect do
        firestore.set document_path, set_data, merge: true
      end.must_raise ArgumentError
