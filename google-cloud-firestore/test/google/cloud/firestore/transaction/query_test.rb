...
describe Google::Cloud::Firestore::Transaction, :query, :mock_firestore do
  ...
  it "runs a simple query" do
    ...
    results_enum = transaction.select(:name).from(:users).run
  ...
  it "runs a complex query" do
    ...
    results_enum = transaction.select(:name).from(:users).offset(3).limit(42).order(:name).order(:__name__, :desc).start_after(:foo).end_before(:bar).run
