...
def setup_isolated_gcloud_config(tmpdir):
    ...
    print("CLOUDSDK_CONFIG is:")
    print(os.environ['CLOUDSDK_CONFIG'])
...
def run_docker(image, env_file, kokoro_root, kokoro_artifacts_dir, build_file):
    ...
    p = Popen("(Get-Location).Path", shell=True, stdout=PIPE, encoding="utf-8", stderr=PIPE)
...
def main():
    ...
    print("Kokoro artifacts dir is:")
    print(kokoro_artifacts_dir)
    kokoro_artifacts_dir = kokoro_artifacts_dir.replace("T:\\", "C:\\").replace("t:\\", "C:\\")
    print("changed kokoro artifacts dir to")
    print(kokoro_artifacts_dir)
    kokoro_gfile_dir = os.environ['KOKORO_GFILE_DIR']
    print("Printing where gcloud")
    print(subprocess.check_call(["where", "gcloud"]))
    print("Kokoro gfile dir is:")
    print(kokoro_gfile_dir)
    print("It's contents are:")
    print("\n".join(os.listdir(kokoro_gfile_dir)))
    old_service_account_key_file = os.path.join(
        kokoro_gfile_dir, 'kokoro-trampoline.service-account.json')
    service_account_key_file = os.path.join(
        kokoro_gfile_dir, 'kokoro-trampoline-service-account.json')
    os.rename(old_service_account_key_file, service_account_key_file)
    print("Service account key file is:")
    print(service_account_key_file)
    print(os.path.isfile(service_account_key_file))
