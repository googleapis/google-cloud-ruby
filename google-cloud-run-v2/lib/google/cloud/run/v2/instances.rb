# frozen_string_literal: true

# Copyright 2026 Google LLC
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#     https://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.

# Auto-generated by gapic-generator-ruby. DO NOT EDIT!

require "gapic/common"
require "gapic/config"
require "gapic/config/method"

require "google/cloud/run/v2/version"

require "google/cloud/run/v2/instances/credentials"
require "google/cloud/run/v2/instances/paths"
require "google/cloud/run/v2/instances/operations"
require "google/cloud/run/v2/instances/client"
require "google/cloud/run/v2/instances/rest"

module Google
  module Cloud
    module Run
      module V2
        ##
        # The Cloud Run Instances API allows you to manage Cloud Run Instances.
        #
        # @example Load this service and instantiate a gRPC client
        #
        #     require "google/cloud/run/v2/instances"
        #     client = ::Google::Cloud::Run::V2::Instances::Client.new
        #
        # @example Load this service and instantiate a REST client
        #
        #     require "google/cloud/run/v2/instances/rest"
        #     client = ::Google::Cloud::Run::V2::Instances::Rest::Client.new
        #
        module Instances
        end
      end
    end
  end
end

helper_path = ::File.join __dir__, "instances", "helpers.rb"
require "google/cloud/run/v2/instances/helpers" if ::File.file? helper_path
