...
describe Google::Cloud::Spanner::Session, :execute, :streaming, :mock_spanner do
  ...
  it "can execute a query with Time param" do
    ...
    mock = Minitest::Mock.new
    mock.expect :execute_streaming_sql, results_enum, [session.path, "SELECT * FROM users WHERE updated_at = @updated_at", transaction: nil, params: Google::Protobuf::Struct.new(fields: { "updated_at" => Google::Protobuf::Value.new(string_value: "2017-01-02T03:04:05Z") }), param_types: { "updated_at" => Google::Spanner::V1::Type.new(code: :TIMESTAMP) }]
    session.service.mocked_service = mock
  ...
  it "can execute a query with IO-ish param" do
    ...
    mock = Minitest::Mock.new
    mock.expect :execute_streaming_sql, results_enum, [session.path, "SELECT * FROM users WHERE avatar = @avatar", transaction: nil, params: Google::Protobuf::Struct.new(fields: { "avatar" => Google::Protobuf::Value.new(string_value: "contents") }), param_types: { "avatar" => Google::Spanner::V1::Type.new(code: :BYTES) }]
    session.service.mocked_service = mock
