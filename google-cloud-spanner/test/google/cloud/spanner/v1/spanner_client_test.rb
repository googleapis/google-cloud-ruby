...
describe Google::Cloud::Spanner::V1::SpannerClient do
  ...
  describe 'execute_batch_dml' do
    custom_error = CustomTestError_v1.new "Custom test error for Google::Cloud::Spanner::V1::SpannerClient#execute_batch_dml."

    it 'invokes execute_batch_dml without error' do
      # Create request parameters
      formatted_session = Google::Cloud::Spanner::V1::SpannerClient.session_path("[PROJECT]", "[INSTANCE]", "[DATABASE]", "[SESSION]")
      transaction = {}
      statements = []
      ...
      # Create expected grpc response
      expected_response = {}
      expected_response = Google::Gax::to_proto(expected_response, Google::Spanner::V1::ExecuteBatchDmlResponse)

      # Mock Grpc layer
      mock_method = proc do |request|
        assert_instance_of(Google::Spanner::V1::ExecuteBatchDmlRequest, request)
        assert_equal(formatted_session, request.session)
        assert_equal(Google::Gax::to_proto(transaction, Google::Spanner::V1::TransactionSelector), request.transaction)
        statements = statements.map do |req|
          Google::Gax::to_proto(req, Google::Spanner::V1::ExecuteBatchDmlRequest::Statement)
        end
        assert_equal(statements, request.statements)
        assert_equal(seqno, request.seqno)
        OpenStruct.new(execute: expected_response)
      end
      mock_stub = MockGrpcClientStub_v1.new(:execute_batch_dml, mock_method)

      # Mock auth layer
      mock_credentials = MockSpannerCredentials_v1.new("execute_batch_dml")
      ...
      Google::Spanner::V1::Spanner::Stub.stub(:new, mock_stub) do
        Google::Cloud::Spanner::V1::Credentials.stub(:default, mock_credentials) do
          ...
          # Call method
          response = client.execute_batch_dml(
            formatted_session,
            transaction,
            statements,
            seqno
    ...
    it 'invokes execute_batch_dml with error' do
      # Create request parameters
      formatted_session = Google::Cloud::Spanner::V1::SpannerClient.session_path("[PROJECT]", "[INSTANCE]", "[DATABASE]", "[SESSION]")
      transaction = {}
      statements = []
