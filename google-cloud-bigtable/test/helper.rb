...
class BigtableServiceWithMock < Google::Cloud::Bigtable::Service
  attr_accessor :mocked_service, :mocked_instances, :mocked_tables

  def service
    mocked_service || super
  end
...
class MockBigtable < Minitest::Spec
  ...
  def app_profie_hash \
      name: nil,
      description: nil,
      single_cluster_routing: nil,
      multi_cluster_routing: nil
    {
      name: name,
  ...
  def app_profiles_hash instance_id, num: 3, start_id: 1
    app_profiles = num.times.map do |i|
      app_profie_hash(
        name: app_profile_path(instance_id, "app-profile-#{start_id + i}"),
  ...
  def snapshots_hash instance_id, num: 3, start_id: 1
    snapshots = num.times.map do |i|
      {
        name: snapshot_path(instance_id, "cluster-#{i}", "snapshot-#{i + start_id}"),
        source_table: Google::Bigtable::Admin::V2::Table.new(
          name: table_path(instance_id, "table-#{i + start_id}"),
        ),
        data_size_bytes: 1024,
        state: :READY,
        description: "Test snapshot - #{i + start_id}",
...
class BigtableTestError < StandardError
  def initialize operation_name
    super("Custom test error #{operation_name}")
  end
end
